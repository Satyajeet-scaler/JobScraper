...
def _infinite_scroll_root(page: Page) -> ElementHandle | None:
    try:
        els = page.query_selector_all("div.infinite-scroll-component")
        if not els:
            return None
        for el in els:
            try:
                oy = el.evaluate("el => window.getComputedStyle(el).overflowY")
...
def _grid_children_cards(scroll_root: ElementHandle) -> list[ElementHandle]:
    ...
    try:
        ...
        grid = scroll_root.query_selector("div[class*='grid']")
        if grid:
            children = grid.query_selector_all("xpath=./div")
            if children:
                return children
    except Exception:
        pass
    ...
    try:
        return scroll_root.query_selector_all(
            "xpath=.//div[contains(@class,'relative')][.//a[contains(@href,'/viewjob/')]]"
        )
    except Exception:
        return []


def _viewjob_hrefs_in_card(page: Page, card: ElementHandle) -> set[str]:
    try:
        hrefs = card.evaluate(
            """
            (c) => {
...
def _click_carousel_next(page: Page, card: ElementHandle) -> bool:
    """
...
def expand_carousels_in_card(page: Page, card: ElementHandle) -> set[str]:
    """
    ...
    for i in range(MAX_CLICKS):
        collected |= _viewjob_hrefs_in_card(page, card)
        if not _click_carousel_next(page, card):
            break
        _pause_after_carousel_next_click()
        new_batch = _viewjob_hrefs_in_card(page, card)
...
def _structural_dedupe_card_indices(page: Page, cards: list[ElementHandle]) -> list[int] | None:
    """
    ...
    try:
        # Playwright's evaluate can take ElementHandles as arguments
...
def _visible_job_dedupe_keys_for_cards(page: Page, cards: list[ElementHandle]) -> list[str]:
    """
...
def _dedupe_card_elements(page: Page, cards: list[ElementHandle]) -> list[ElementHandle]:
    """
    Deduplicate card elements structurally and by visible job content.
    """
    if len(cards) <= 1:
        return cards

    indices = _structural_dedupe_card_indices(page, cards)
    if indices is None:
        logger.warning(
    ...
    keys = _visible_job_dedupe_keys_for_cards(page, structural)
    seen: set[str] = set()
...
def _filter_cards_in_viewport(page: Page, cards: list[ElementHandle]) -> list[ElementHandle]:
    """
...
def list_viewport_company_cards(page: Page) -> list[ElementHandle]:
    """
    Company card roots in the infinite-scroll feed that are **in the current viewport**.
    """
    root = _infinite_scroll_root(page)
    if not root:
        return []

    cards = _grid_children_cards(root)
    cards = _dedupe_card_elements(page, cards)
    ...
    for card in cards:
        try:
            ...
            continue
        try:
            has_job = card.query_selector('a[href*="/viewjob/"]')
            has_btns = card.query_selector("button")
    ...
    return _filter_cards_in_viewport(page, candidates)


def _card_label(page: Page, card: ElementHandle) -> str:
...
def expand_all_visible_company_cards_detailed(page: Page) -> list[CardJobsRow]:
    """
    Process all visible company cards and expand their carousels.
    """
    root = _infinite_scroll_root(page)
    if not root:
    ...
    for idx, card in enumerate(cards):
        ...
        part = expand_carousels_in_card(page, card)
