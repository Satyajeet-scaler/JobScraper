...
def fetch_unclassified_linkedin_posts(
    *,
    requested_role: str,
    run_date: date | str,
    limit: int = 500,
    ...
    SELECT
        ...
        raw_payload_json, classify_done
    FROM linkedin_posts
    WHERE requested_role = %s
      AND run_date = %s
      AND classify_done = FALSE
    ORDER BY id ASC
    LIMIT %s
    """
    with _db() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (role, run_date, limit))
            rows = cur.fetchall() or []
...
def count_unclassified_linkedin_posts(*, requested_role: str, run_date: date | str) -> int:
    role = (requested_role or "").strip()
    if isinstance(run_date, str):
        run_date = _to_date(run_date)
    sql = """
    SELECT COUNT(*) AS c
    FROM linkedin_posts
    WHERE requested_role = %s AND run_date = %s AND classify_done = FALSE
    """
    with _db() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (role, run_date))
    ...
    with _db() as conn:
        with conn.cursor() as cur:
            ...
            if not post_id:
                ...
                row_res = cur.fetchone() or {}
                post_id = int(row_res.get("id") or 0)
    if post_id:
        logger.info("Successfully upserted linkedin_post id=%d url_normalized=%s", post_id, post_url_normalized)
    else:
...
def upsert_linkedin_post_relevance(row: dict[str, Any]) -> None:
    """Upsert classification results into linkedin_post_relevance.

    Requires ``linkedin_post_id`` in the row dict. Callers must ensure the parent
    post exists (e.g. via upsert_linkedin_post) before calling this.
    """
    post_id = int(row.get("linkedin_post_id") or row.get("id") or 0)
    if not post_id:
        logger.warning("upsert_linkedin_post_relevance: missing linkedin_post_id in row")
        return
...
def fetch_unsent_relevant_linkedin_posts_for_run_date(
    run_date: str,
) -> list[dict[str, Any]]:
    """Fetch all relevant + unsent posts for a run date (global/original pipeline)."""
    sql = """
    SELECT
...
def fetch_unsent_relevant_linkedin_posts_for_role(
    *,
    ...
    """Fetch relevant + unsent rows for a specific role and run date."""
    sql = """
    SELECT
        lp.id AS linkedin_post_id,
        lp.*,
